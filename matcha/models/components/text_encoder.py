## Github[text_encoder.py]: https://github.com/shivammehta25/Matcha-TTS/blob/main/matcha/models/components/text_encoder.py
import math
import numpy as np
...
# from matcha.utils.models import sequence_mask
...
class ConvReluNorm(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, kernel_size, n_layers, p_dropout):
        ...
        self.conv_layers = nn.ModuleList() ## torch.nn.ModuleList()
        self.norm_layers = nn.ModuleList() ## torch.nn.ModuleList()
...
class RotaryPositionalEmbeddings(nn.Module):
    ...
    def __init__(self, d: int, base: int = 10_000):
        """
        * `d`: the number of features $d$
        * `base`: the constant used for calculating $\Theta$
        """
        super().__init__()
    ...
    def forward(self, x: torch.Tensor):
        """
        * `x`: the Tensor at the head of a key or a query with shape
...
class Encoder(nn.Module):
    def __init__(self, hidden_channels, filter_channels, n_heads, n_layers, kernel_size=1, p_dropout = 0.0, **kwargs):
    ...
    def forward(self, x, x_mask):
        ...
        for i in range(self.n_layers):
            ...
            y = self.norm_layers_1[i](x + y)
            y = self.ffn_layers[i](x, x_mask)
            y = self.drop(y)

            y = self.norm_layers_2[i](x + y)
    ...
    def __init__(self,
        ...
        if encoder_params.prenet:
            self.prenet = ConvReluNorm(self.n_channels, self.n_channels, self.n_channels,
                                       kernel_size = 5, n_layers = 3, p_dropout = .5
                                       )
        else:
